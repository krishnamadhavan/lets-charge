...
| Application / product code | Raw ingest, product schema, Schneider profile + projector. No driver/operator UI. |
...
5. If they say “just continue” with no pick, **PR 6** is next (CitrineOS start/stop + Authorization adapter). Do not open Xcode/Android Studio. Do not start operator/resident UI. Do not treat the lab charger session as the real-SKU merge gate.
