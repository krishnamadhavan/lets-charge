...
| Application / product code | Ingest, schema, projector, CitrineOS start/stop adapter. No driver/operator UI. |
...
5. If they say “just continue” with no pick, **PR 7** is next (Exicom profile as a dialect diff) or **PR 8** (admin/OTP auth). Do not open Xcode/Android Studio. Do not start operator/resident UI. Do not treat the lab charger session as the real-SKU merge gate.
